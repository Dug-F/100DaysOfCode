...
const callbackMakeTonic = (event) => {
  ...
  const clickedElement = document.querySelector(`.innerLabel.bubble[data-position="${position}"]`);
  updatePositionNumber(container, ".note-arm, .bubble.note", -Number(position));
  // updatePositionNumber(container, ".innerLabel-arm, .bubble.innerLabel", -Number(position));
  // populateBubbles("innerLabel", innerLabels[circleStatus.currentScale][circleStatus.currentInnerLabels]);
...
const callbackIntervals = (event) => {
  ...
  for (let key in innerLabels.major) {
    ...
    menuItem.addEventListener("click", changeInnerLabels);
  }
  setContextSubmenuPosition(event);
};

const changeInnerLabels = (event) => {
  circleStatus.currentInnerLabels = event.target.textContent;
  populateBubbles("innerLabel", innerLabels[circleStatus.currentScale][circleStatus.currentInnerLabels])
};
