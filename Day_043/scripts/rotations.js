...
export const gsapAnimateBubbleRotation = (bubble, offset) => {
  ...
  let rotationDegrees = (gsap.getProperty(bubble.parentElement, "rotation") + offset * 30) % 360;
  gsap.to(bubble.parentElement, { rotation: `${rotationDegrees}${rotationDirection}`, duration: 2 });
  gsap.to(bubble, {
...
/**
 * @description rotates the label structure by the passed number of offset positions
 * @param {number} offset Number of positions to rotate the label -12 >= number <= 12
 * @returns {NodeList} list of label bubble nodes rotated or null if offset is 0, or +-12
 */
export const rotateLabel = (offset) => {
  ...
  const labelBubbles = document.querySelectorAll(`.label.bubble`).forEach((bubble) => {
...
export const rotateNoteEvent = (clickedElem) => {
  ...
  rotateNote(-selectedPosition);
}


/**
 * @description rotates the note structure by the passed number of offset positions
 * @param {number} offset Number of positions to rotate the note -12 >= number <= 12
 * @returns {NodeList} list of note bubble nodes rotated or null if offset is 0, or +-12
 */
export const rotateNote = (offset) => {
  offset = (Number(offset) + 12) % 12
  if (offset === 0) {
    return null;
  }
  ...
  const noteBubbles = document.querySelectorAll(`.note.bubble`).forEach((bubble) => {
    // animate the rotation
    gsapAnimateBubbleRotation(bubble, offset);
  });
