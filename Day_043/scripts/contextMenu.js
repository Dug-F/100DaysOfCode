...
import { updatePositionNumbers, updateHomeOffset, populateBubbles, getElementRect } from "./helperFunctions.js";
...
const callbackMakeTonic = (event) => {
  // get current position number of clicked element
  ...
  // update current position number of note elements by subtracting current positionon number of clicked element
  // so that clicked element becomes new position 0
  updatePositionNumbers(container, ".note-arm, .bubble.note, .connector", -position);
  // update status home offset with offset of new position 0
  
  console.log("status.homeOffset: ", status.homeOffset, " position: ", position);
  updateHomeOffset(Number(position) + status.homeOffset);
  // remove chord diagrams
  status.chordShapeType = "No chord";
  drawChords(position);
  rotateLabel(position);
};
...
const callbackClock = (event) => {
  ...
  const targetPosition = 12 - status.homeOffset;
    // console.log("rotation: ", homeBubble.style.transform);
  // console.log(status);
  // const homeLabel = document.querySelector(`.label.bubble[data-position="${status.homeOffset}"]`);
  updatePositionNumbers(container, ".note-arm, .bubble.note, .connector", -Number(targetPosition));
  // const clickedElement = document.querySelector(`.label.bubble[data-position="${-Number(position)}"]`);
  // // status.chordShapeType = "No chord";
  // // drawChords(position);
  rotateNote(homeBubble);
  rotateLabel(targetPosition);
  status.homeOffset = 0;
};
